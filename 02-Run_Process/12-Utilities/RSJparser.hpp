...
int is_bracket (char c, std::vector<char const*>& bracks, int indx=0) {
    for (unsigned int b = 0; b < bracks.size(); ++b)
        if (c==bracks[b][indx]) 
            return (b);
    return (-1);
}
...
public:
    ...
    RSJresource& operator[] (unsigned int indx); // array
    ...
    void parse (const std::string& data, RSJresourceType typ = RSJ_UNKNOWN) {
        ...
        if (typ==RSJ_OBJECT || typ==RSJ_UNKNOWN) {
            ...
            if (content.length() != data.length()) { // a valid object
                ...
                for (unsigned int a=0; a<nvPairs.size(); ++a) {
...
RSJresource& RSJresource::operator[] (unsigned int indx) { // returns reference
    as_array();
...
std::string  RSJresource::as<std::string> (const std::string& def) {
    ...
    for (unsigned int a=0; a < escapes.size(); ++a)
