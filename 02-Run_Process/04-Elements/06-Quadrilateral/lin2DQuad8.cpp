...
lin2DQuad8::lin2DQuad8(const std::vector<unsigned int> nodes, std::unique_ptr<Material> &material, const double th, const std::string quadrature, const unsigned int nGauss, bool massform) :
Element("lin2DQuad8", nodes, 16, VTKCELL), t(th), MassForm(massform) {
    //The element nodes.
    theNodes.resize(8);
...
Eigen::MatrixXd 
lin2DQuad8::GetStrainAt(double x3, double x2) const{
...
Eigen::MatrixXd 
lin2DQuad8::GetStressAt(double x3, double x2) const{
    UNUNSED_PARAMETER(x3);
    UNUNSED_PARAMETER(x2);
...
Eigen::VectorXd 
lin2DQuad8::ComputeStrainRate(const Eigen::MatrixXd &Bij) const{
