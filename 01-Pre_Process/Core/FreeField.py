...
def Compute2DFreeFieldBoundaries(x, y, vals, dt, xmin, layers, beta, rho, nu, option, df=0.2, cof=30.0):
    '''
    ...
    ----------
    x  : array
        Vector with the x-coordinatesto compute the free-field forces
    y  : array
        Vector with the y-coordinatesto compute the free-field forces
    vals  : array
    ...
    #Transform time-series in frequency domain and computes variables required to compute reponse layer interface
    nt = len(vals)
...
def Compute3DFreeFieldBoundaries(x, y, vals, dt, xmin, layers, beta, rho, nu, option, df=0.2, cof=30.0):
    pass
